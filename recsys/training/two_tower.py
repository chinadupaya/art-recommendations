...
class QueryTowerFactory:
    ...
    def build(
        ...
        return QueryTower(
            ...
            # gender_groups=self._dataset.properties["gender_groups"],
            emb_dim=embed_dim,
        )
...
class QueryTower(tf.keras.Model):
    def __init__(
        ...
        # self.gender_group_tokenizer = StringLookup(
        #     vocabulary=gender_groups,
        #     mask_token=None,
        # )
    ...
    def call(self, inputs):
        # gender_group_embedding = tf.one_hot(
        #     self.gender_group_tokenizer(inputs["gender"]),
        #     len(self.gender_groups),
        # )
        concatenated_inputs = tf.concat(
            [
                self.user_embedding(inputs["user_id"]),
                tf.reshape(self.normalized_age(inputs["age"]), (-1, 1)),
                # gender_group_embedding
            ],
            axis=1,
        )
...
class ItemTowerFactory:
    ...
    def build(
        ...
        return ItemTower(
            ...
            category_groups=self._dataset.properties["category_groups"],
            emb_dim=embed_dim,
        )
...
class ItemTower(tf.keras.Model):
    def __init__(
        ...
        # Converts strings into integer indices (scikit-learn LabelEncoder analog)
        self.category_group_tokenizer = StringLookup(
            vocabulary=category_groups,
            mask_token=None,
        )
    ...
    def call(self, inputs):
        category_group_embedding = tf.one_hot(
            self.category_group_tokenizer(inputs["category"]),
            len(self.category_groups),
        )

        concatenated_inputs = tf.concat(
            [
                self.item_embedding(inputs["artwork_id"]),
                category_group_embedding,
            ],
            axis=1,
        )
...
class TwoTowerDataset:
    ...
    def get_train_val_split(self):
        ...
        self._properties = {
            ...
            "item_ids": train_df["artwork_id"].unique().tolist(),
            "category_groups": train_df["category"].unique().tolist(),
            # "gender_groups": train_df["gender"].unique().tolist(),
        }
